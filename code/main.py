...
embedding, word2idx = process.load_embedding("corpus.word2vec")#加载embeddings
Q = [] #问题
T = [] #正确答案
F = [] #错误选项
stop_words = codecs.open("data/unused.txt", 'r', encoding='utf8').readlines()
for i in range(0,len(stop_words)):
    stop_words[i]=stop_words[i].strip()
process.prepare_data("data/database.txt", "data/train.txt", stop_words)
...
#加载训练集和测试集合
train_questions, train_answers, train_labels, train_question_num = process.load_data("data/database.txt", "data/train.txt", word2idx, stop_words, train_sim_ixs, max_len)
test_questions, test_answers, test_labels, test_question_num = process.load_data("data/database.txt", "data/test.txt", word2idx, stop_words, test_sim_ixs, max_len)
for q, ta, fa in process.pre_epoch(
        train_questions, train_answers, train_labels, train_question_num, length): Q.append(q), T.append(ta), F.append(fa)
...
def update_params():
    ...
    for i in range(shr_rate):
        ...
        for epoch in range(num_epochs):
            for question, trueAnswer, falseAnswer in zip(Q, T, F):
                feed_dict = {
...
with tf.Graph().as_default(), tf.device("/gpu:0"):
    ...
    with tf.compat.v1.Session(config=session_conf).as_default() as sess:
        ...
        #计算梯度下降参数
        grads, _ = tf.compat.v1.clip_by_global_norm(tf.gradients(lstm.loss, tvars), iter_ub)
